...
def get_movers(symbols, date_obj=None, time_obj=None, live=False):
    ...
    if live:
        ...
        try:
            ...
            data = r.json()
            stock_map = {s.get("symbol"): s for s in data.get("data", [])}
...
def sector_menu(chat_id):
    buttons = [
        [{"text": "🏦 Bank", "callback_data": "sector_BANK"}, {"text": "💻 IT", "callback_data": "sector_IT"}, {"text": "🚗 Auto", "callback_data": "sector_AUTO"}],
        [{"text": "💊 Pharma", "callback_data": "sector_PHARMA"}, {"text": "🛒 FMCG", "callback_data": "sector_FMCG"}, {"text": "⚙️ Metal", "callback_data": "sector_METAL"}],
        [{"text": "🏢 Realty", "callback_data": "sector_REALTY"}, {"text": "⚡ Energy", "callback_data": "sector_ENERGY"}, {"text": "💰 Fin Serv", "callback_data": "sector_FINSERV"}],
        [{"text": "🏛️ PSU Bank", "callback_data": "sector_PSUBANK"}, {"text": "📊 All FnO", "callback_data": "sector_ALL"}]
    ]
    ...
    try:
        ...
        if "callback_query" in update:
            ...
            if data.startswith("sector_"):
                ...
                send_msg(chat_id, build_message(gainers, losers, breadth, date_label=today, sector=None if sector == "ALL" else sector, nifty_change=nifty_change))
        ...
        if text_lower == "/sectors":
            ...
            if not args:
                ...
                if not rest:
                    ...
                    gainers, losers, breadth = get_movers(symbols, live=True)
                    send_msg(chat_id, build_message(gainers, losers, breadth, date_label=today, sector=sector_key, nifty_change=get_nifty_change()))
                else:
                    ...
                    send_msg(chat_id, build_message(gainers, losers, breadth, date_label=date_label, time_label=time_label, sector=sector_key))
            ...
            if not args:
                ...
                today = datetime.datetime.now(IST).strftime("%d %b %Y")
                send_msg(chat_id, build_message(gainers, losers, breadth, date_label=today, nifty_change=get_nifty_change()))
                ...
                send_msg(chat_id, build_message(gainers, losers, breadth, date_label=date_label, time_label=time_label))
